...
export class AuthManager {
    constructor() {
        // 🌟 优先使用登录页面的 token 和 account_id
        this.loginToken = localStorage.getItem('auth_token');
        this.loginAccountId = localStorage.getItem('account_id');
        
        if (this.loginToken && this.loginAccountId) {
            this.accountId = this.loginAccountId;
            // 🌟 确保登录账号也有助记词
            this.mnemonic = localStorage.getItem('player_mnemonic');
            if (!this.mnemonic) {
        ...
        // 回退到助记词方式
        this.mnemonic = storageAdapter.get(STORAGE_KEYS.MNEMONIC);
        if (!this.mnemonic) {
            this.mnemonic = generateMnemonic();
