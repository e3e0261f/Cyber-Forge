...
class AudioEngine {
    ...
    setBGMVolume(v) {
        ...
        if (this.musicGain) {
            const target = Math.max(0, this.bgmVolume * 0.35);
            this.musicGain.gain.setValueAtTime(target || 0.0001, this.ctx ? this.ctx.currentTime : 0);
            // BGM 音量为 0 时暂停背景音乐
            if (this.bgmVolume === 0 && this.bgmTimer) {
                this.stopBGM();
            } else if (this.bgmVolume > 0 && !this.bgmTimer && this.unlocked) {
                // BGM 音量从 0 恢复时自动重启播放
                this.startBGM();
            }
        }
    ...
    /** 批量应用外部设置 (用于页面加载时从 localStorage 恢复设置) */
    applySettings(masterVol, sfxVol, bgmVol) {
        if (typeof masterVol === 'number') this.volume = Math.max(0, Math.min(1, masterVol));
        if (typeof sfxVol === 'number') this.sfxVolume = Math.max(0, Math.min(1, sfxVol));
        if (typeof bgmVol === 'number') this.bgmVolume = Math.max(0, Math.min(1, bgmVol));
        if (this.masterGain) this.masterGain.gain.value = this.volume;
        if (this.sfxGain) this.sfxGain.gain.value = this.sfxVolume;
    }
    ...
    startBGM() {
        ...
        // 渐进淡入音乐音量 (尊重用户设置的 bgmVolume)
        const targetVol = Math.max(0.0001, this.bgmVolume * 0.35);
        this.musicGain.gain.setValueAtTime(0.0001, this.ctx.currentTime);
        this.musicGain.gain.exponentialRampToValueAtTime(targetVol, this.ctx.currentTime + 3);
