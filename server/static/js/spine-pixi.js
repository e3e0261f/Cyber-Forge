...
var spine = (() => {
  ...
  // 🌟 修复版：无条件接管 require，且永远优先返回全局的 window.PIXI
  if (typeof window !== "undefined") {
    const prevRequire = window.getComputedStyle ? window.require : undefined;
    window.require = (x) => {
      if (x && (x.startsWith("@pixi/") || x.startsWith("pixi.js") || x === "pixi.js")) {
        return window.PIXI;
      }
      if (typeof prevRequire === "function") {
  ...
  var Interpolation = class _Interpolation {
    ...
    apply(start, end, a) {
      if (end === void 0 || a === void 0)
        return this.applyInternal(start);
  ...
  // spine-core/dist/TextureAtlas.js
  ...
  // spine-core/dist/attachments/Attachment.js
  ...
  var VertexAttachment = class _VertexAttachment extends Attachment {
    ...
    computeWorldVertices(skeleton, slot, start, count, worldVertices2, offset, stride) {
  ...
  // spine-core/dist/attachments/MeshAttachment.js
  ...
  // spine-core/dist/attachments/RegionAttachment.js
  var RegionAttachment = class _RegionAttachment extends Attachment {
     ...
     * @param offset The `worldVertices` index to begin writing values.
     * @param stride The number of `worldVertices` entries between the value pairs written. */
    computeWorldVertices(slot, vertexOffsets, worldVertices2, offset, stride) {
      const bone = slot.bone.appliedPose;
  ...
  // spine-core/dist/attachments/Sequence.js
  ...
  var SlotCurveTimeline = class extends CurveTimeline {
    ...
    apply(skeleton, lastTime, time, events, alpha, from, add, out, appliedPose) {
      ...
      if (slot.bone.active)
        this.apply1(slot, appliedPose ? slot.appliedPose : slot.pose, time, alpha, from, add);
    }
  };
  ...
  var DeformTimeline = class extends CurveTimeline {
    ...
    applyBeforeFirst(slot, appliedPose, alpha, from) {
      if (!slot.bone.active)
        return;
      const pose = appliedPose ? slot.appliedPose : slot.pose;
      if (pose.attachment == null || pose.attachment.timelineAttachment !== this.attachment)
        return;
  ...
  var DrawOrderTimeline = class _DrawOrderTimeline extends Timeline {
    static propertyID = `${Property.drawOrder}`;
  ...
  var DrawOrderFolderTimeline = class _DrawOrderFolderTimeline extends Timeline {
    static propertyID = `${Property.drawOrderFolder}`;
    slots;
    inFolder;
    drawOrders;
  ...
  // spine-core/dist/AnimationState.js
  var AnimationState = class _AnimationState {
    ...
    applyMixingFrom(to, skeleton) {
      ...
      for (let i = 0; i < timelineCount; i++) {
        ...
        if ((mode & HOLD) !== 0) {
          ...
          if (!drawOrder && timeline instanceof DrawOrderTimeline && mixFrom === MixFrom.current)
            continue;
          alpha = alphaMix;
        }
        ...
        if (!shortestRotation && timeline instanceof RotateTimeline) {
          ...
          this.applyAttachmentTimeline(timeline, skeleton, applyTime, mixFrom, retainAttachments && alpha >= from.alphaAttachmentThreshold);
        else {
    ...
    eventsReverse(entry, animationLast, animationTime) {
      ...
      for (let i = 0, n = entry.animation.timelines.length; i < n; i++) {
        ...
        if (!(eventTimeline instanceof EventTimeline))
          continue;
    ...
    clearTrack(trackIndex) {
      if (trackIndex < 0)
        throw new Error("trackIndex must be >= 0.");
  ...
  var TrackEntry = class {
    ...
    mix() {
      ...
      if (mix >= 1)
        return 1;
      if (this.mixInterpolation === Interpolation.linear)
        return mix;
  ...
  // spine-core/dist/AnimationStateData.js
  ...
  // spine-core/dist/AssetManagerBase.js
  var AssetManagerBase = class {
    ...
    reuseAssets(path2, success = () => {
      ...
      const loadedStatus = this.cache.getAsset(path2);
    ...
    loadTexture(path2, success = () => {
      ...
      if (this.reuseAssets(path2, success, error))
        return;
      const pma = this.texturePmaInfo[path2];
      this.cache.assetsLoaded[path2] = new Promise((resolve, reject) => {
        ...
        if (isWebWorker) {
          fetch(path2, { mode: "cors" }).then((response) => {
            ...
            if (bitmap) {
              const texture = this.createTexture(path2, pma, bitmap);
          ...
          image.onload = () => {
            const texture = this.createTexture(path2, pma, image);
            this.success(success, path2, texture);
            resolve(texture);
          };
          image.onerror = () => {
    ...
    loadTextureAtlas(path2, success = () => {
      ...
      const parent = index >= 0 ? path2.substring(0, index + 1) : "";
      path2 = this.start(path2);
      if (this.reuseAssets(path2, success, error))
        return;
      this.cache.assetsLoaded[path2] = new Promise((resolve, reject) => {
        this.downloader.downloadText(path2, (atlasText) => {
          try {
            const atlas = this.createTextureAtlas(atlasText, parent, path2, fileAlias);
            ...
            for (const page of atlas.pages) {
              this.loadTexture(this.texturePath(parent, page.name, fileAlias), (imagePath, texture) => {
                if (!abort) {
    ...
    loadTextureAtlasButNoTextures(path2, success = () => {
      ...
      this.cache.assetsLoaded[path2] = new Promise((resolve, reject) => {
        this.downloader.downloadText(path2, (atlasText) => {
          try {
            const atlas = this.createTextureAtlas(atlasText, parent, path2);
            this.success(success, path2, atlas);
            resolve(atlas);
          } catch (e) {
            const errorMsg = `Couldn't parse texture atlas ${path2}: ${e.message}`;
...
${error}` : ""}`);
    ...
    removeAll() {
      for (const path2 in this.cache.assets) {
        const asset = this.cache.assets[path2];
        if (asset.dispose)
          asset.dispose();
      }
    ...
    createTextureAtlas(atlasText, parentPath, path2, fileAlias) {
  ...
  var AssetCache = class _AssetCache {
    ...
    getAsset(path2) {
      return this.assetsLoaded[path2];
    }
  };
  ...
  // spine-core/dist/attachments/BoundingBoxAttachment.js
  ...
  // spine-core/dist/attachments/ClippingAttachment.js
  ...
  // spine-core/dist/attachments/PathAttachment.js
  ...
  // spine-core/dist/attachments/PointAttachment.js
  ...
  // spine-core/dist/AtlasAttachmentLoader.js
  var AtlasAttachmentLoader = class {
     ...
     * raised. */
    findRegion(name, path2) {
      const region = this.atlas.findRegion(path2);
      if (!region && !this.allowMissingRegions)
        throw new Error(`Region not found in atlas: ${path2} (attachment: ${name})`);
      return region;
    }
    newRegionAttachment(skin, placeholder, name, path2, sequence) {
      this.findRegions(name, path2, sequence);
      return new RegionAttachment(name, sequence);
    }
    newMeshAttachment(skin, placeholder, name, path2, sequence) {
      this.findRegions(name, path2, sequence);
  ...
  var BonePose = class {
    ...
    set(pose) {
      if (pose == null)
        throw new Error("pose cannot be null.");
      this.x = pose.x;
      this.y = pose.y;
    ...
    localToWorld(local) {
      if (local == null)
        throw new Error("local cannot be null.");
  ...
  // spine-core/dist/PosedActive.js
  ...
  // spine-core/dist/ConstraintData.js
  ...
  // spine-core/dist/IkConstraintPose.js
  ...
  // spine-core/dist/IkConstraint.js
  var IkConstraint = class _IkConstraint extends Constraint {
    ...
    update(skeleton, physics) {
      ...
      switch (bones.length) {
        case 1:
          ...
          break;
        case 2:
          _IkConstraint.apply(skeleton, bones[0], bones[1], target.worldX, target.worldY, p.bendDirection, p.stretch, this.data.scaleYMode, p.softness, p.mix);
          break;
      }
    }
    sort(skeleton) {
    ...
    static apply(skeleton, boneOrParent, targetXorChild, targetYOrTargetX, compressOrTargetY, stretchOrBendDir, scaleYModeOrStretch, mixOrScaleYMode, softness, mix) {
      if (typeof targetXorChild === "number")
        ...
        _IkConstraint.apply2(skeleton, boneOrParent, targetXorChild, targetYOrTargetX, compressOrTargetY, stretchOrBendDir, scaleYModeOrStretch, mixOrScaleYMode, softness, mix);
    }
    ...
    static apply2(skeleton, parent, child, targetX, targetY, bendDir, stretch, scaleYMode, softness, mix) {
      ...
      if (l1 < MathUtils.epsilon) {
        _IkConstraint.apply(skeleton, parent, targetX, targetY, false, stretch, ScaleYMode.None, mix);
        child.rotation = 0;
        return;
      }
      x = targetX - pp.worldX;
  ...
  // spine-core/dist/IkConstraintData.js
  ...
  // spine-core/dist/PathConstraintPose.js
  ...
  // spine-core/dist/PathConstraintData.js
  ...
  // spine-core/dist/PathConstraint.js
  var PathConstraint = class _PathConstraint extends Constraint {
    ...
    computeWorldPositions(skeleton, path2, spacesCount, tangents) {
        ...
        for (let i = 0, o = 0, curve = 0; i < spacesCount; i++, o += 3) {
          ...
          if (closed) {
            ...
            if (prevCurve !== _PathConstraint.BEFORE) {
              ...
              path2.computeWorldVertices(skeleton, slot, 2, 4, world, 0, 2);
            }
            ...
            if (prevCurve !== _PathConstraint.AFTER) {
              ...
              path2.computeWorldVertices(skeleton, slot, verticesLength - 6, 4, world, 0, 2);
            }
          ...
          if (curve !== prevCurve) {
            ...
            if (closed && curve === curveCount) {
              path2.computeWorldVertices(skeleton, slot, verticesLength - 4, 4, world, 0, 2);
              path2.computeWorldVertices(skeleton, slot, 0, 4, world, 4, 2);
            } else
              path2.computeWorldVertices(skeleton, slot, curve * 6 + 2, 8, world, 0, 2);
          }
      ...
      if (closed) {
        ...
        path2.computeWorldVertices(skeleton, slot, 2, verticesLength - 4, world, 0, 2);
        path2.computeWorldVertices(skeleton, slot, 0, 2, world, verticesLength - 4, 2);
        ...
        verticesLength -= 4;
        world = Utils.setArraySize(this.world, verticesLength);
        path2.computeWorldVertices(skeleton, slot, 2, verticesLength, world, 0, 2);
      }
  ...
  // spine-core/dist/PhysicsConstraintPose.js
    ...
    getBounds(offset, size, temp = new Array(2), clipper2 = null) {
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        if (attachment) {
          ...
          } else if (attachment instanceof ClippingAttachment && clipper2) {
            clipper2.clipEnd(slot);
            clipper2.clipStart(this, slot, attachment);
            continue;
          }
          if (vertices && triangles) {
            if (clipper2?.isClipping() && clipper2.clipTriangles(vertices, triangles, triangles.length)) {
  ...
  // spine-core/dist/PhysicsConstraint.js
  ...
  // spine-core/dist/PhysicsConstraintData.js
  ...
  // spine-core/dist/SkeletonData.js
  ...
  // spine-core/dist/TransformConstraintPose.js
  ...
  // spine-core/dist/TransformConstraint.js
  ...
  // spine-core/dist/TransformConstraintData.js
  ...
  // spine-core/dist/SkeletonBinary.js
    ...
    readAttachment(input, skeletonData, skin, slotIndex, placeholder, nonessential) {
      ...
      switch (flags & 7) {
        ...
        case AttachmentType.Region: {
          ...
          if (!path2)
            path2 = name;
          const region = this.attachmentLoader.newRegionAttachment(skin, placeholder, name, path2, sequence);
          if (!region)
            return null;
          region.path = path2;
          region.x = x * scale;
        ...
        case AttachmentType.Mesh: {
          ...
          if (!path2)
            path2 = name;
          const mesh = this.attachmentLoader.newMeshAttachment(skin, placeholder, name, path2, sequence);
          if (!mesh)
            return null;
          mesh.path = path2;
        ...
        case AttachmentType.Path: {
          ...
          const color = nonessential ? input.readInt32() : 0;
          const path2 = this.attachmentLoader.newPathAttachment(skin, placeholder, name);
          if (!path2)
            return null;
          path2.closed = closed;
    ...
    readAnimation(input, name, skeletonData, nonessential) {
      ...
      for (let i = 0, n = input.readInt(true); i < n; i++) {
        ...
        for (let ii = 0, nn = input.readInt(true); ii < nn; ii++) {
          ...
          switch (type) {
            case PATH_POSITION:
              readTimeline(input, timelines, new PathConstraintPositionTimeline(frameCount, bezierCount, index), data.positionMode === PositionMode.Fixed ? scale : 1);
              break;
            case PATH_SPACING:
              readTimeline(input, timelines, new PathConstraintSpacingTimeline(frameCount, bezierCount, index), data.spacingMode === SpacingMode.Length || data.spacingMode === SpacingMode.Fixed ? scale : 1);
              break;
            case PATH_MIX: {
  ...
  // spine-core/dist/SkeletonBounds.js
  ...
  // spine-core/dist/Triangulator.js
  var Triangulator = class _Triangulator {
          ...
          outer: if (!isConcave[i]) {
            ...
            for (let ii = next + 1 < vertexCount ? next + 1 : 0; ii !== previous; ) {
              if (isConcave[ii]) {
                ...
                if (_Triangulator.positiveArea(p3x, p3y, p1x, p1y, vx, vy) && _Triangulator.positiveArea(p1x, p1y, p2x, p2y, vx, vy) && _Triangulator.positiveArea(p2x, p2y, p3x, p3y, vx, vy))
                  break outer;
              }
              if (++ii === vertexCount)
                ii = 0;
    ...
    static isConcave(index, vertexCount, vertices, indices) {
      ...
      return !_Triangulator.positiveArea(vertices[previous], vertices[previous + 1], vertices[current], vertices[current + 1], vertices[next], vertices[next + 1]);
    }
  ...
  // spine-core/dist/SkeletonClipping.js
  var SkeletonClipping = class {
    ...
    clipStart(skeleton, slot, clip) {
      ...
      if (convex || this.inverse || clip.convex) {
        if (!convex)
          this.makeConvex(clippingPolygon);
    ...
    clipEnd(slot) {
      if (!this.clipAttachment)
        return;
      if (slot && this.clipAttachment.endSlot !== slot.data)
        return;
  ...
  // spine-core/dist/SkeletonJson.js
  var SkeletonJson = class {
    ...
    readAttachment(map, skin, slotIndex, placeholder, skeletonData) {
      ...
      switch (getValue(map, "type", "region")) {
        case "region": {
          const path2 = getValue(map, "path", name);
        ...
        case "linkedmesh": {
          ...
          if (source) {
            ...
            this.linkedMeshes.push(new LinkedMesh2(mesh, getValue(map, "skin", null), slotIndex, sourceIndex, source, getValue(map, "timelines", true)));
            return mesh;
          }
          const uvs = map.uvs;
    ...
    readAnimation(map, name, skeletonData) {
      ...
      if (map.ik) {
        for (const constraintName in map.ik) {
          ...
          const timeline = new IkConstraintTimeline(constraintMap.length, constraintMap.length << 1, skeletonData.constraints.indexOf(constraint));
          let time = getValue(keyMap, "time", 0);
      ...
      if (map.transform) {
        for (const constraintName in map.transform) {
          ...
          const timeline = new TransformConstraintTimeline(timelineMap.length, timelineMap.length * 6, skeletonData.constraints.indexOf(constraint));
          let time = getValue(keyMap, "time", 0);
          ...
          for (const timelineName in constraintMap) {
            ...
            switch (timelineName) {
              case "position": {
                ...
                readTimeline12(timelines, timelineMap, timeline, 0, constraint.positionMode === PositionMode.Fixed ? scale : 1);
                break;
              }
              case "spacing": {
                const timeline = new PathConstraintSpacingTimeline(frames, frames, index);
                readTimeline12(timelines, timelineMap, timeline, 0, constraint.spacingMode === SpacingMode.Length || constraint.spacingMode === SpacingMode.Fixed ? scale : 1);
                break;
              }
              case "mix": {
        ...
        for (const attachmentsName in map.attachments) {
          ...
          for (const slotMapName in attachmentsMap) {
            ...
            for (const attachmentMapName in slotMap) {
              ...
              if (!attachment)
                throw new Error(`Timeline attachment not found: ${attachmentMapName}`);
  ...
  // spine-core/dist/SkeletonPhysicsMovement.js
  var SkeletonPhysicsMovement = class {
    ...
    applyTransformMovement() {
      ...
      if (!inheritPosition && !inheritRotation)
        return;
      const transform = this.currentTransform;
      ...
      if (this.hasLastTransform) {
        if (!positionChanged && currentRotation === this.lastRotation)
          return;
        if (inheritPosition && positionChanged)
          this.applyPositionMovement(x, y, z);
        if (inheritRotation && currentRotation !== this.lastRotation)
          this.applyRotationMovement(currentRotation);
      }
  ...
  // spine-core/dist/SkeletonRendererCore.js
    ...
    render(skeleton, pma = false, inColor, stride = 2, slotZOffset = 0) {
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        if (attachment instanceof RegionAttachment) {
          ...
          clipper2.clipEnd(slot);
          clipper2.clipStart(skeleton, slot, attachment);
          continue;
        } else {
          clipper2.clipEnd(slot);
          continue;
        ...
        if (clipper2.isClipping()) {
          clipper2.clipTrianglesUnpacked(vertices, 0, indices, indicesCount, uvs, stride);
          vertices = clipper2.clippedVerticesTyped;
          verticesCount = clipper2.clippedVerticesLength / stride;
          uvs = clipper2.clippedUVsTyped;
          indices = clipper2.clippedTrianglesTyped;
          indicesCount = clipper2.clippedTrianglesLength;
        }
  ...
  var SkeletonRenderer = class _SkeletonRenderer {
    ...
    drawTriangles(skeleton) {
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        if (attachment instanceof RegionAttachment) {
          ...
          texture = (sequence.regions[sequenceIndex]?.texture).getImage();
        } else if (attachment instanceof MeshAttachment) {
          const sequence = attachment.sequence;
          const sequenceIndex = sequence.resolveIndex(pose);
          const uvs = sequence.getUVs(sequenceIndex);
    ...
    computeRegionVertices(slot, region, offsets, uvs, pma) {
      ...
      region.computeWorldVertices(slot, offsets, this.vertices, 0, _SkeletonRenderer.VERTEX_SIZE);
    ...
    computeMeshVertices(slot, mesh, uvs, pma) {
      ...
      mesh.computeWorldVertices(skeleton, slot, 0, mesh.worldVerticesLength, vertices, 0, _SkeletonRenderer.VERTEX_SIZE);
  ...
  var SpinePipe = class {
    ...
    validateRenderable(spine) {
      if (this.renderer.type === import_pixi7.RendererType.CANVAS) {
        return true;
      }
  ...
  var Spine = class _Spine extends import_pixi8.ViewContainer {
    ...
    _lastAttachments = [];
    _lastRenderableCacheIds = [];
    _lastClippingAttachmentIds = [];
    _stateChanged = true;
    attachmentCacheData = [];
    get debug() {
      return this._debug;
    ...
    updateAndSetPixiMask(slot) {
      ...
      if (currentClippingSlot && slot.bone.active && slotObject?.wasRenderable) {
        ...
        const clippingAttachment = slotClipping.appliedPose.attachment;
    ...
    closeCurrentPixiMaskIfEndingAt(slot) {
      const currentClippingSlot = this.currentClippingSlot;
    ...
    cleanupPixiMasks() {
      for (const key in this.clippingSlotToPixiMasks) {
        ...
        if ((!clippingSlotToPixiMask.slot.bone.active || !(clippingSlotToPixiMask.slot.appliedPose.attachment instanceof ClippingAttachment) || !clippingSlotToPixiMask.maskComputed) && clippingSlotToPixiMask.mask) {
        ...
        if (attachment) {
          if (attachment instanceof MeshAttachment || attachment instanceof RegionAttachment) {
            ...
            if (this.alpha === 0 || alpha === 0) {
              ...
              const texture = sequence.regions[sequenceIndex]?.texture?.texture || import_pixi8.Texture.EMPTY;
    ...
    updateSlotObject(slotAttachment) {
      ...
      if (renderable) {
        ...
        if (slotAttachment.followSlotColor) {
          container.tint = 255 * this.skeleton.color.r * pose.color.r << 16 | 255 * this.skeleton.color.g * pose.color.g << 8 | 255 * this.skeleton.color.b * pose.color.b;
        }
      } else if (container.mask) {
        container.mask = null;
      }
    }
    isSlotObjectRenderable(slotAttachment, slotAlpha = this.skeleton.color.a * slotAttachment.slot.appliedPose.color.a) {
      ...
      return this.skeleton.drawOrder.appliedPose.includes(slot) && slot.bone.active && followAttachmentValue && this.alpha > 0 && slotAlpha > 0;
    }
    getRenderableCacheId(slot, attachment) {
      return `${slot.data.index}-${attachment.name}`;
    }
    getExistingCachedData(slot, attachment) {
      return this.attachmentCacheData[slot.data.index]?.[attachment.name];
    }
    markSlotAttachmentSkipped(slot, attachment) {
    ...
    unloadFromCache() {
      ...
      if (!cacheKey || !import_pixi8.Cache.has(cacheKey)) return false;
      if (import_pixi8.Cache.get(cacheKey) !== skeletonData) return false;
      import_pixi8.Cache.remove(cacheKey);
    ...
    static createOptions({ skeleton, atlas, scale = 1, darkTint, autoUpdate = true, boundsProvider, allowMissingRegions = false, ticker }) {
      ...
      if (import_pixi8.Cache.has(cacheKey)) {
        const skeletonData2 = import_pixi8.Cache.get(cacheKey);
        _Spine.skeletonDataCacheKeys.set(skeletonData2, cacheKey);
        return {
