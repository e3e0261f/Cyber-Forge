...
pub fn save_dir() -> PathBuf {
    if let Ok(p) = std::env::var("CYBER_FORGE_SAVE_DIR") {
        if !p.is_empty() {
            return PathBuf::from(p);
        }
    }
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".local/share/cyber-forge");
    }
    if let Ok(appdata) = std::env::var("APPDATA") {
        return PathBuf::from(appdata).join("cyber-forge");
    }
    PathBuf::from(".")
}
...
impl GameState {
    pub fn to_save_json(&self) -> Option<String> {
        ...
        serde_json::to_string(&SavePayload {
            data,
    ...
    pub fn from_save_json(s: &str) -> Option<Self> {
        ...
        let mut h = Sha256::new();
        h.update(p.data.as_bytes());
        if format!("{:x}", h.finalize()) != p.hash {
            return None;
        }
