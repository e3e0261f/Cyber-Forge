...
impl GameState {
    ...
    pub fn stamp_backpack_cooldown(&mut self) {
        ...
        for s in &mut self.backpack { if !s.is_tool { s.forged_timestamp = now; } }
    }
    pub fn toggle_auto_melt(&mut self) {
        self.melt_tier = self.melt_tier.next();
        self.stamp_backpack_cooldown();
        self.set_toast(format!("自动熔炼档位：{}（3秒后生效）", self.melt_tier.name()));
    }
    pub fn toggle_auto_list(&mut self) {
        self.list_tier = self.list_tier.next();
        self.stamp_backpack_cooldown();
        self.set_toast(format!("自动上架门槛：{}（3秒后生效，可连按G调档）", self.list_tier.name()));
    }
    ...
    pub fn process_auto_melt(&mut self) {
        ...
        while i < self.backpack.len() {
            ...
            if sword.quality.rank() <= max_target_rank && now.saturating_sub(sword.forged_timestamp) >= 3 {
                ...
                let cult = ((melt_sword.price as f64).sqrt() as u128 / 10).max(1).min(80);
    ...
    pub fn add_iron_slag(&mut self, amount: u32) {
        ...
        if self.iron_slag >= 1000 {
            ...
            self.push_log(format!("铁浆炼气：1000 铁浆凝融，气感 +{}（现气感 {}）", conversions, self.realm.body.qi_sense), false, false);
        }
    }
    pub fn grant_visitor_slag(&mut self, buyer_title: &str, element_match: bool, impulsive: bool, deal_closed: bool) {
        ...
        let mut gain = base;
        if element_match { gain = ((gain as f64) * 1.5).round() as u32; }
        if impulsive { gain = ((gain as f64) * 1.8).round() as u32; }
        if deal_closed { gain = gain.saturating_add(base / 2 + 1); }
        if gain == 0 { return; }
        self.add_iron_slag(gain);
        if deal_closed || gain >= 8 {
            let tag = if element_match && impulsive { "五行特需·斗气" }
                else if element_match { "五行特需" }
                else if impulsive { "斗气冲动" } else { "道韵残留" };
            self.push_log(format!("铁浆增益：{} 过境留下 {} 铁浆（{}）", buyer_title, gain, tag), false, false);
        }
    }
